...
const ChooseUser = ({ visitor }) => {
  ...
  const navigateHandler = async (user) => {
    if (visitor === "guest") {
      setLoader(true);
      try {
        // Always seed guest accounts first, wait for it to complete
    ...
    if (user === "Admin") {
      if (visitor === "guest") {
        dispatch(loginUser({ email: "guestadmin@gmail.com", password }, user));
      } else {
        navigate('/Adminlogin');
      }
    } else if (user === "Student") {
      if (visitor === "guest") {
        dispatch(loginUser({ rollNum: "1", studentName: "Guest Student", password }, user));
      } else {
