...
import {
    Typography, Grid, Box, Avatar, Paper, Button, Container,
...
const TeacherProfile = () => {
    ...
    React.useEffect(() => {
        if (currentUser) {
            setFormData({
                ...
                gender: currentUser.gender || '', password: ''
            });
        }
    }, [currentUser]);
    ...
    const infoFields = [
        ...
        { label: 'Subject', value: currentUser.teachSubject?.subName || 'Not assigned' },
        { label: 'School', value: currentUser.school?.schoolName || 'Not assigned' },
    ];
            ...
            <Box sx={{ p: 3, background: '#F8FAFC', minHeight: '100vh' }}>
                ...
                <Container maxWidth="md" disableGutters>
                    <Paper elevation={0} sx={{ p: 3, mb: 3, borderRadius: '12px', border: '1px solid #E2E8F0', background: '#fff', textAlign: 'center' }}>
                        ...
                        <Typography sx={{ fontSize: 13, color: '#64748B', mt: 0.5 }}>{currentUser.email}</Typography>
                        {currentUser.teachSclass && (
                            <Typography sx={{ fontSize: 13, color: '#64748B' }}>
                                Class: {Array.isArray(currentUser.teachSclass)
                                    ? currentUser.teachSclass.map(c => c.sclassName).join(', ')
