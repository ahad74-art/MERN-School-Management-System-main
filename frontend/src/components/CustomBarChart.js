...
const CustomTooltipContent = ({ active, payload, dataKey }) => {
    if (active && payload && payload.length) {
        ...
        console.log('🔍 Tooltip dataKey:', dataKey);
            ...
            <CustomTooltip>
                {dataKey === "attendancePercentage" ? (
                    <>
                        ...
                        <TooltipText>Attended: ({data.attendedClasses}/{data.totalClasses})</TooltipText>
                        <TooltipText>{data.attendancePercentage}%</TooltipText>
                    </>
                ) : dataKey === "marksObtained" ? (
                    <>
                        <TooltipMain>{data.subject}</TooltipMain>
                        <TooltipText>Marks: {data.marksObtained}</TooltipText>
                        {data.subName && data.subName.subCode && (
                            <TooltipText>Code: {data.subName.subCode}</TooltipText>
                        )}
                    </>
                ) : (
                    <>
                        <TooltipMain>{data.subject || 'Unknown'}</TooltipMain>
                        <TooltipText>Value: {data[dataKey]}</TooltipText>
                    </>
                )}
            </CustomTooltip>
        );
...
const CustomBarChart = ({ chartData, dataKey }) => {
    ...
    console.log('📊 CustomBarChart received data:', chartData);
    console.log('📊 CustomBarChart dataKey:', dataKey);

    const subjects = chartData.map((data) => data.subject);
    const distinctColors = generateDistinctColors(subjects.length);

    // Determine the correct dataKey for XAxis based on data structure
    let xAxisDataKey = "subject";
    if (dataKey === "marksObtained") {
        // For marks, use subject field
        xAxisDataKey = "subject";
    } else if (dataKey === "attendancePercentage") {
        // For attendance, use subject field
        xAxisDataKey = "subject";
    }
    ...
    if (dataKey === "attendancePercentage") {
        yAxisDomain = [0, 100];
    } else if (dataKey === "marksObtained") {
        yAxisDomain = [0, 100]; // Assuming marks are out of 100
    }
